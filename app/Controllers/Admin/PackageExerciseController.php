  ...
  function list($f3)
  {
    ...
    try {
      ...
      if($bodyPartId == null){
        ...
        // show exercises in package selected and not selected
        $query = '
          ...
          FROM (
            ...
            ' . ($bodyPartId != null ? ' WHERE body_part_id = ' . $bodyPartId : '') . '
          ) T
          LEFT JOIN 
          (
            SELECT 
            ...
            INNER JOIN body_parts BP ON BP.id = E.body_part_id
            INNER JOIN packages_exercises PE ON PE.exercise_id = E.id
            WHERE PE.package_id = %d ' . ($bodyPartId != null ? ' AND body_part_id = ' . $bodyPartId : '') . '
          ) P 
