...
class LevelController extends BaseController
{
  ...
  function save($f3)
  {
    ...
    try {
      ...
      if(count($news) > 0){
				foreach ($news as &$new) {
				  $n = \Model::factory('App\\Models\\Level', 'app')->create();
					$n->name = $new['name'];
					$n->save();
				  $temp = [];
				  $temp['tempId'] = $new['id'];
      ...
      if(count($edits) > 0){
          ...
          $e = \Model::factory('App\\Models\\Level', 'app')->find_one($edit['id']);
					$e->name = $edit['name'];
					$e->save();
        }
      }
      ...
      if(count($deletes) > 0){
			    ...
			    $d = \Model::factory('App\\Models\\Level', 'app')->find_one($delete['id']);
			    $d->delete();
				}
      }
      // commit
